...
variable "topic-name" {
  type        = string
  default     = "sentinel-cloudrun-topic"
...
resource "google_pubsub_topic" "sentinel-cloudrun-topic" {
  count   = "${var.topic-name != "sentinel-cloudrun-topic" ? 0 : 1}"
  name    = var.topic-name
  project = data.google_project.project.project_id
}
...
resource "google_pubsub_subscription" "sentinel-subscription" {
  ...
  depends_on = [google_pubsub_topic.sentinel-cloudrun-topic]
}
